...
#
# recent Raspian versions can't boot the created image, because the UUID/PUUID of the backup differs 
#
# Get UUDI and PARTUUID from the original:
#
# root@raspberrypi:/home/pi# blkid /dev/mmcblk0p2
#                                       
# /dev/mmcblk0p2: LABEL="rootfs" UUID="5fa1ec37-3719-4b25-be14-1f7d29135a13" TYPE="ext4" PARTUUID="c862a254-02"
#                                      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^                        ^^^^^^^^
#
#  UUID=5fa1ec37-3719-4b25-be14-1f7d29135a13
#  PUUID=c862a254
#
# setup loopback device
#
# export IMG=/media/pi/BACKUP/MyRaspi.img 
# LOOPBACK=$(losetup -f) 
# losetup $LOOPBACK $IMG
# partx --add  $LOOPBACK
#
# if needed  2fsck -f -y ${LOOPBACK}p2
#
# export DEVICE=${LOOPBACK}
# tune2fs $LOOPBACK -U <original UUID>
#
# PUUID 
#    PUID of MBR partitiontables is the MBR identifier with appended partitionnumber.
#    PARTUUID="c862a254-02" ->  0xc862a254
#
# fdisk $LOOPBACK
# 	x (expert)
